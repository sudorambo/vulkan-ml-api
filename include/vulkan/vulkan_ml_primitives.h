...
static const VkPipelineStageFlags2 VK_PIPELINE_STAGE_2_ML_GRAPH_BIT_KHR = 0x100000000ULL;

/* VkAccessFlagBits2 extensions */
static const VkAccessFlags2 VK_ACCESS_2_ML_GRAPH_READ_BIT_KHR = 0x100000000ULL;
static const VkAccessFlags2 VK_ACCESS_2_ML_GRAPH_WRITE_BIT_KHR = 0x200000000ULL;

/* VkFormatFeatureFlagBits2 extensions */
static const VkFormatFeatureFlags2 VK_FORMAT_FEATURE_2_TENSOR_SHADER_BIT_KHR = 0x100000000ULL;
...
typedef enum VkTensorTilingKHR {
    VK_TENSOR_TILING_OPTIMAL_KHR = 0, /**< Implementation-selected opaque layout; applications must
                                         not assume element arrangement. */
    VK_TENSOR_TILING_LINEAR_KHR = 1,  /**< Row-major order with application-specified or
                                         densely-packed strides; memory may be mapped. */
...
typedef enum VkTensorUsageFlagBitsKHR {
    VK_TENSOR_USAGE_SHADER_BIT_KHR =
        0x00000001, /**< May be accessed from compute shaders via SPIR-V tensor accessors. */
    VK_TENSOR_USAGE_TRANSFER_SRC_BIT_KHR =
        0x00000002, /**< May be used as source in vkCmdCopyTensorKHR. */
    VK_TENSOR_USAGE_TRANSFER_DST_BIT_KHR =
        0x00000004, /**< May be used as destination in vkCmdCopyTensorKHR. */
    VK_TENSOR_USAGE_ML_GRAPH_INPUT_BIT_KHR =
        0x00000008, /**< May be bound as input to an ML graph dispatch. */
    VK_TENSOR_USAGE_ML_GRAPH_OUTPUT_BIT_KHR =
        0x00000010, /**< May be bound as output of an ML graph dispatch. */
    VK_TENSOR_USAGE_ML_GRAPH_WEIGHT_BIT_KHR =
        0x00000020, /**< May be bound as constant weight/bias parameter of an ML graph. */
    VK_TENSOR_USAGE_IMAGE_ALIASING_BIT_KHR =
        0x00000040, /**< May alias device memory with a VkImage; requires tensorImageAliasing
                       feature. */
...
typedef enum VkMLTensorBindingTypeKHR {
    VK_ML_TENSOR_BINDING_TYPE_INTERNAL_KHR =
        0, /**< Produced by another node; nodeIndex identifies producer. */
    VK_ML_TENSOR_BINDING_TYPE_EXTERNAL_INPUT_KHR =
        1, /**< Provided as graph input; tensorIndex into pExternalInputDescriptions. */
    VK_ML_TENSOR_BINDING_TYPE_EXTERNAL_OUTPUT_KHR =
        2, /**< Written as graph output; tensorIndex into pExternalOutputDescriptions. */
    VK_ML_TENSOR_BINDING_TYPE_EXTERNAL_WEIGHT_KHR =
        3, /**< Constant weight; tensorIndex into pConstantWeightDescriptions. */
...
/** @brief Physical device ML feature support. Chain into VkPhysicalDeviceFeatures2 or
 * VkDeviceCreateInfo. */
...
/** @brief Describes tensor geometry, element format, memory layout, and usage. Used when creating
 * tensors and defining graph bindings. */
...
/** @brief Parameters for creating a tensor view. Pass to vkCreateTensorViewKHR. Enables format
 * reinterpretation and sub-region access. */
...
/** @brief Identifies a tensor for memory requirements query. Pass to
...
/** @brief Tensor memory barrier for synchronization. Chain into VkDependencyInfo or
...
/** @brief Collection of tensor memory barriers. Chain into VkDependencyInfo for
...
/** @brief Writes tensor views to a descriptor set. Chain into VkWriteDescriptorSet for
...
/** @brief Tensor-specific format features. Chain into VkFormatProperties3 when calling
...
/** @brief Descriptor for 2D convolution and deconvolution. Use with
 * VK_ML_OPERATION_TYPE_CONVOLUTION_2D_KHR or DECONVOLUTION_2D_KHR. */
...
/** @brief Descriptor for GEMM: D = alpha * op(A) * op(B) + beta * C. Use with
 * VK_ML_OPERATION_TYPE_GEMM_KHR or FULLY_CONNECTED_KHR. */
...
/** @brief Descriptor for batch normalization or LRN. Use with
 * VK_ML_OPERATION_TYPE_BATCH_NORMALIZATION_KHR or LRN_KHR. */
...
/** @brief Descriptor for element-wise add or multiply. Use with
 * VK_ML_OPERATION_TYPE_ELEMENTWISE_ADD_KHR or ELEMENTWISE_MUL_KHR. */
...
typedef struct VkMLPrimitiveDescTransposeKHR {
    ...
    const uint32_t *pPermutation; /**< Permutation indices; pPermutation[i] is the source dim for
                                     output dim i. */
...
/** @brief Defines one node in an ML graph. Specifies operation type, descriptor, and input/output
 * bindings. */
...
/** @brief Parameters for creating an ML graph. Pass to vkCreateMLGraphKHR. Defines nodes and
 * external inputs/outputs/weights. */
...
/** @brief Parameters for creating an ML session. Pass to vkCreateMLSessionKHR. Associates graph
 * with scratch memory. */
...
/** @brief Parameters for dispatching an ML graph. Pass to vkCmdDispatchMLGraphKHR. Binds input,
 * output, and weight tensors. */
 ...
 * @note All submitted commands that refer to tensor must have completed. All VkTensorViewKHR from
 * tensor must be destroyed first.
 */
 ...
 * @return VK_SUCCESS, or VK_ERROR_OUT_OF_HOST_MEMORY, VK_ERROR_OUT_OF_DEVICE_MEMORY.
 * @note Each tensor must not already be bound. Memory must satisfy requirements from
 * vkGetTensorMemoryRequirementsKHR.
 */
 ...
 * @note Must be called outside a render pass. Source and destination must have TRANSFER_SRC and
 * TRANSFER_DST usage respectively.
 */
 ...
 * @param pAllocator Controls host memory deallocation; must match allocation used at creation.
 * @note All submitted commands referencing graph must have completed. All VkMLSessionKHR must be
 * destroyed first.
 */
 ...
 * @param pMemoryRequirements Pointer to VkMemoryRequirements2 in which requirements are returned.
 * @note If mlGraphScratchAutoAllocation is enabled, scratch may be allocated internally; otherwise
 * bind via session.
 */
 ...
 * @return VK_SUCCESS, or VK_ERROR_OUT_OF_HOST_MEMORY, VK_ERROR_OUT_OF_DEVICE_MEMORY.
 * @note Scratch memory size must satisfy vkGetMLGraphMemoryRequirementsKHR. If scratchMemory is
 * VK_NULL_HANDLE, mlGraphScratchAutoAllocation must be enabled.
 */
 ...
 * @note Command buffer must be in recording state. Input/output/weight counts must match the
 * graph's external input/output/constant weight counts.
 */
