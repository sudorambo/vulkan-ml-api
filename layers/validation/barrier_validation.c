...
{
    const VkAccessFlags2 VALID_TENSOR_ACCESS_MASK =
    ...
    if (!barrier)
        return VK_FALSE;
    if ((uint32_t)barrier->sType != VK_STRUCTURE_TYPE_TENSOR_MEMORY_BARRIER_KHR)
        return VK_FALSE;
...
VkBool32 vk_ml_validate_tensor_dependency_info(const VkTensorDependencyInfoKHR *depInfo)
{
    if (!depInfo)
        return VK_FALSE;
    if ((uint32_t)depInfo->sType != VK_STRUCTURE_TYPE_TENSOR_DEPENDENCY_INFO_KHR)
        return VK_FALSE;
